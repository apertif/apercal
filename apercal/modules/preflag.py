...
class preflag:
    ...
    def shadow(self):
        ...
        preflagtargetbeamsshadow = get_param_def(self, 'preflag_targetbeams_shadow',
                                                 np.full((beams), False))  # Are the target beams shadow flagged?
        ...
        if self.preflag_shadow:
            ...
            if preflagfluxcalshadow:
                ...
                if self.fluxcal != '' and os.path.isdir(
                        self.basedir + '00' + '/' + self.rawsubdir + '/' + self.fluxcal):
                    logger.debug('Flagging shadowed antennas for flux calibrator #')
                    fc_shadow = 'flagdata(vis="' + self.basedir + '00' + '/' + self.rawsubdir + '/' + self.fluxcal + \
                                '", mode="shadow", flagbackup=False)'
                    casacmd = [fc_shadow]
                    ...
                    logger.warning(
                        'Flux calibrator dataset not specified or dataset not available. Not flagging shadowed '
                        'antennas for flux calibrator #')
            ...
            if preflagpolcalshadow:
                ...
                if self.polcal != '' and os.path.isdir(self.basedir + '00' + '/' + self.rawsubdir + '/' + self.polcal):
                    ...
                    pc_shadow = 'flagdata(vis="' + self.basedir + '00' + '/' + self.rawsubdir + '/' + self.polcal + \
                                '", mode="shadow", flagbackup=False)'
                    casacmd = [pc_shadow]
                    casa = drivecasa.Casapy()
                    casa.run_script(casacmd, raise_on_severe=False, timeout=1800)
                    ...
                    logger.warning(
                        'Polarised calibrator dataset not specified or dataset not available. Not flagging shadowed '
                        'antennas for polarised calibrator #')
            ...
            if self.target != '':
                ...
                for vis in datasets:
                    if preflagtargetbeamsshadow[int(vis.split('/')[-3])]:
                        logger.info('Shadowed antenna(s) for beam ' + vis.split('/')[-3] + ' were already flagged #')
                    else:
                        logger.debug('Flagging shadowed antennas for beam ' + vis.split('/')[-3] + ' #')
    ...
    def edges(self):
        ...
        preflagtargetbeamsedges = get_param_def(self, 'preflag_targetbeams_edges',
                                                np.full((beams), False))  # Edges of target beams flagged?
        ...
        if self.preflag_edges:
            ...
            if preflagfluxcaledges:
                ...
                if self.fluxcal != '' and os.path.isdir(
                    ...
                    fc_edges_flagcmd = 'flagdata(vis="' + self.basedir + '00' + '/' + self.rawsubdir + '/' + \
                                       self.fluxcal + '", spw="0:' + m + '", flagbackup=False)'
                    ...
                    logger.warning(
                        'No flux calibrator dataset specified. Subband edges of flux calibrator will not be flagged! #')
            ...
            if preflagpolcaledges:
                ...
                if self.polcal != '' and os.path.isdir(self.basedir + '00' + '/' + self.rawsubdir + '/' + self.polcal):
                    ...
                    l = a + b + c
                    m = ';'.join(str(ch) for ch in l)
                    pc_edges_flagcmd = 'flagdata(vis="' + self.basedir + '00' + '/' + self.rawsubdir + '/' + \
                                       self.polcal + '", spw="0:' + m + '", flagbackup=False)'
            ...
            if self.target != '':
                ...
                for vis in datasets:
                    if preflagtargetbeamsedges[int(vis.split('/')[-3])]:
                        logger.info('Subband edges for target beam ' + vis.split('/')[-3] + ' were already flagged #')
                    else:
                        logger.debug('Flagging subband edges for target beam ' + vis.split('/')[-3] + ' #')
    ...
    def ghosts(self):
        ...
        preflagtargetbeamsghosts = get_param_def(self, 'preflag_targetbeams_ghosts',
                                                 np.full((beams), False))  # Ghosts of target beams flagged?
        ...
        if self.preflag_ghosts:
            ...
            if preflagfluxcalghosts:
                ...
                if self.fluxcal != '' and os.path.isdir(
                    ...
                    l = a + b
                    m = ';'.join(str(ch) for ch in l)
                    fc_ghosts_flagcmd = 'flagdata(vis="' + self.basedir + '00' + '/' + self.rawsubdir + '/' + \
                                        self.fluxcal + '", spw="0:' + m + '", flagbackup=False)'
                    ...
                    logger.warning(
                        'No flux calibrator dataset specified. Ghosts in flux calibrator dataset will not be flagged! #')
            ...
            if preflagpolcalghosts:
                ...
                if self.polcal != '' and os.path.isdir(self.basedir + '00' + '/' + self.rawsubdir + '/' + self.polcal):
                    ...
                    l = a + b
                    m = ';'.join(str(ch) for ch in l)
                    pc_ghosts_flagcmd = 'flagdata(vis="' + self.basedir + '00' + '/' + self.rawsubdir + '/' + \
                                        self.polcal + '", spw="0:' + m + '", flagbackup=False)'
                    ...
                    logger.warning(
                        'No polarised calibrator dataset specified. Ghosts in polarised calibrator will '
                        'not be flagged! #')
            ...
            if self.target != '':
                ...
                for vis in datasets:
                    if preflagtargetbeamsghosts[int(vis.split('/')[-3])]:
                        logger.info('Ghost channels for target beam ' + vis.split('/')[-3] + ' were already flagged #')
                    else:
                        logger.debug('Flagging ghost channels for target beam ' + vis.split('/')[-3] + ' #')
                        nchannel = self._getnchan(vis)
        ...
        if self.preflag_manualflag_auto:
            ...
            if preflagfluxcalmanualflagauto and self.preflag_manualflag_fluxcal:
                ...
                if self.preflag_manualflag_fluxcal and os.path.isdir(
                        self.basedir + '00' + '/' + self.rawsubdir + '/' + self.fluxcal):
                    fc_auto = 'flagdata(vis="' + self.basedir + '00' + '/' + self.rawsubdir + '/' + self.fluxcal + \
                    ...
                    logger.warning(
                        'No flux calibrator dataset specified. Auto-correlations for flux calibrator dataset will '
                        'not be flagged! #')
            ...
            if preflagpolcalmanualflagauto and self.preflag_manualflag_polcal:
                ...
                if self.preflag_manualflag_polcal and os.path.isdir(
                        self.basedir + '00' + '/' + self.rawsubdir + '/' + self.polcal):
                    pc_auto = 'flagdata(vis="' + self.basedir + '00' + '/' + self.rawsubdir + '/' + self.polcal + \
                              '", autocorr=True, flagbackup=False)'
                    casacmd = [pc_auto]
                    casa = drivecasa.Casapy()
                    casa.run_script(casacmd, raise_on_severe=False, timeout=1800)
                    ...
                    logger.warning(
                        'No polarised calibrator dataset specified. Auto-correlations for polariased calibrator '
            ...
            if self.preflag_manualflag_target:
                ...
                for vis in datasets:
                    if preflagtargetbeamsmanualflagauto[int(vis.split('/')[-3])]:
                        logger.info(
                            'Auto-correlations for target beam ' + vis.split('/')[-3] + ' were already flagged #')
                    else:
                        logger.debug('Flagging auto-correlations for target beam ' + vis.split('/')[-3] + ' #')
                ...
                logger.warning(
                    'Target dataset not specified. Auto-correlations for target beam dataset(s) will not be flagged! #')
    ...
    def manualflag_antenna(self):
        ...
        preflagfluxcalmanualflagantenna = get_param_def(self, 'preflag_fluxcal_manualflag_antenna',
                                                        np.full((1), '', dtype='U50'))

        # Flagged antennas of polcal?
        preflagpolcalmanualflagantenna = get_param_def(self, 'preflag_polcal_manualflag_antenna',
                                                       np.full((1), '', dtype='U50'))
        ...
        if self.preflag_manualflag_antenna != '':
            ...
            if preflagfluxcalmanualflagantenna[
                0] == self.preflag_manualflag_antenna and self.preflag_manualflag_fluxcal == True:
                logger.info(
                    'Antenna(s) ' + self.preflag_manualflag_antenna + ' for flux calibrator were already flagged #')
            else:
                if self.preflag_manualflag_fluxcal and os.path.isdir(
                        self.basedir + '00' + '/' + self.rawsubdir + '/' + self.fluxcal):
                    fc_ant = 'flagdata(vis="' + self.basedir + '00' + '/' + self.rawsubdir + '/' + self.fluxcal + \
                             '", antenna="' + self.preflag_manualflag_antenna + '", flagbackup=False)'
                    casacmd = [fc_ant]
                    casa = drivecasa.Casapy()
                    casa.run_script(casacmd, raise_on_severe=False, timeout=1800)
                    ...
                    logger.warning(
                        'No flux calibrator dataset specified. Specified antenna(s) for flux calibrator dataset '
                        'will not be flagged! #')
            # Flag antenna(s) for polarised calibrator
            if preflagpolcalmanualflagantenna[
                0] == self.preflag_manualflag_antenna and self.preflag_manualflag_polcal == True:
                logger.info(
                    'Antenna(s) ' + self.preflag_manualflag_antenna + ' for polarised calibrator were already flagged #')
            else:
                if self.preflag_manualflag_polcal and os.path.isdir(
                        self.basedir + '00' + '/' + self.rawsubdir + '/' + self.polcal):
                    pc_ant = 'flagdata(vis="' + self.basedir + '00' + '/' + self.rawsubdir + '/' + self.polcal + \
                             '", antenna="' + self.preflag_manualflag_antenna + '", flagbackup=False)'
                    casacmd = [pc_ant]
                    casa = drivecasa.Casapy()
                    casa.run_script(casacmd, raise_on_severe=False, timeout=1800)
                    logger.debug(
                        'Flagged antenna(s) ' + self.preflag_manualflag_antenna + ' for polarised calibrator #')
                    spltant = self.preflag_manualflag_antenna.split(',')
                    for ant in spltant:
                    ...
                    logger.warning(
                        'No polarised calibrator dataset specified. Specified antenna(s) for polarised calibrator '
                        'dataset will not be flagged! #')
            ...
            if self.preflag_manualflag_target:
                if self.preflag_manualflag_targetbeams == 'all':
                    ...
                    datasets = [self.basedir + str(b).zfill(2) + '/' + self.rawsubdir + '/' + self.target for b in
                                beams]
                    logger.debug(
                        'Flagging antenna(s) ' + self.preflag_manualflag_antenna + ' for selected target beams #')
                for vis in datasets:
                    if preflagtargetbeamsmanualflagantenna[int(vis.split('/')[-3])] == self.preflag_manualflag_antenna:
                        logger.info(
                            'Antenna(s) ' + self.preflag_manualflag_antenna + ' for target beam ' + vis.split('/')[
                                -3] + ' were already flagged #')
                    else:
                        logger.debug('Flagging antenna(s) ' + self.preflag_manualflag_antenna + ' for target beam ' +
                                     vis.split('/')[-3] + ' #')
                        tg_ant = 'flagdata(vis="' + str(
                            vis) + '", antenna="' + self.preflag_manualflag_antenna + '", flagbackup=False)'
                        casacmd = [tg_ant]
                        casa = drivecasa.Casapy()
                        casa.run_script(casacmd, raise_on_severe=False, timeout=1800)
                        spltant = self.preflag_manualflag_antenna.split(',')
                        for ant in spltant:
                            if preflagtargetbeamsmanualflagantenna[int(vis.split('/')[-3])].find(ant) == -1:
                                if preflagtargetbeamsmanualflagantenna[int(vis.split('/')[-3])] == '':
                                    ...
                                    preflagtargetbeamsmanualflagantenna[int(vis.split('/')[-3])] = \
                                    preflagtargetbeamsmanualflagantenna[int(vis.split('/')[-3])] + ',' + ant
            else:
                logger.warning('Target dataset not specified. Specified antenna(s) for target beam dataset(s) will '
                               'not be flagged! #')
    ...
    def manualflag_corr(self):
        ...
        preflagfluxcalmanualflagcorr = get_param_def(self, 'preflag_fluxcal_manualflag_corr',
                                                     np.full((1), '', dtype='U50'))  # Flagged correlations of fluxcal?
        preflagpolcalmanualflagcorr = get_param_def(self, 'preflag_polcal_manualflag_corr',
                                                    np.full((1), '', dtype='U50'))  # Flagged correlations of polcal?
        preflagtargetbeamsmanualflagcorr = get_param_def(self, 'preflag_targetbeams_manualflag_corr',
                                                         np.full((beams), '',
                                                                 dtype='U50'))  # Flagged correlations of target beams?
        ...
        if self.preflag_manualflag_corr != '':
            ...
            if preflagfluxcalmanualflagcorr[
                0] == self.preflag_manualflag_corr and self.preflag_manualflag_fluxcal == True:
                logger.info(
                    'Correlation(s) ' + self.preflag_manualflag_corr + ' for flux calibrator were already flagged #')
            else:
                if self.preflag_manualflag_fluxcal and os.path.isdir(
                        self.basedir + '00' + '/' + self.rawsubdir + '/' + self.fluxcal):
                    subs_setinit.setinitdirs(self)
                    fc_corr = 'flagdata(vis="' + self.basedir + '00' + '/' + self.rawsubdir + '/' + self.fluxcal + \
                              '", correlation="' + self.preflag_manualflag_corr + '", flagbackup=False)'
                    casacmd = [fc_corr]
                    casa = drivecasa.Casapy()
                    casa.run_script(casacmd, raise_on_severe=False, timeout=1800)
                    ...
                    logger.warning(
                        'No flux calibrator dataset specified. Specified correlation(s) for flux calibrator dataset '
                        'will not be flagged! #')
            # Flag correlation(s) for flux calibrator
            if preflagpolcalmanualflagcorr[
                0] == self.preflag_manualflag_corr and self.preflag_manualflag_polcal == True:
                logger.info(
                    'Correlation(s) ' + self.preflag_manualflag_corr + ' for polarised calibrator were already flagged #')
            else:
                if self.preflag_manualflag_polcal and os.path.isdir(
                        self.basedir + '00' + '/' + self.rawsubdir + '/' + self.polcal):
                    subs_setinit.setinitdirs(self)
                    pc_corr = 'flagdata(vis="' + self.basedir + '00' + '/' + self.rawsubdir + '/' + self.polcal + \
                              '", correlation="' + self.preflag_manualflag_corr + '", flagbackup=False)'
                    casacmd = [pc_corr]
                    casa = drivecasa.Casapy()
                    casa.run_script(casacmd, raise_on_severe=False, timeout=1800)
                    logger.debug(
                        'Flagged correlation(s) ' + self.preflag_manualflag_corr + ' for polarised calibrator #')
                    spltcorr = self.preflag_manualflag_corr.split(',')
                    for corr in spltcorr:
                    ...
                    logger.warning('No polarised calibrator dataset specified. Specified correlation(s) for polarised '
            ...
            if self.preflag_manualflag_target:
                if self.preflag_manualflag_targetbeams == 'all':
                    ...
                    datasets = [self.basedir + str(b).zfill(2) + '/' + self.rawsubdir + '/' + self.target for b in
                                beams]
                    logger.debug(
                        'Flagging correlation(s) ' + self.preflag_manualflag_corr + ' for selected target beams #')
                for vis in datasets:
                    if preflagtargetbeamsmanualflagcorr[int(vis.split('/')[-3])] == self.preflag_manualflag_corr:
                        logger.info(
                            'Correlation(s) ' + self.preflag_manualflag_corr + ' for target beam ' + vis.split('/')[
                                -3] + ' were already flagged #')
                    else:
                        logger.debug('Flagging correlations(s) ' + self.preflag_manualflag_corr + ' for target beam ' +
                                     vis.split('/')[-3] + ' #')
                        tg_corr = 'flagdata(vis="' + str(
                            vis) + '", correlation="' + self.preflag_manualflag_corr + '", flagbackup=False)'
                        casacmd = [tg_corr]
                        casa = drivecasa.Casapy()
                        casa.run_script(casacmd, raise_on_severe=False, timeout=1800)
                        spltcorr = self.preflag_manualflag_corr.split(',')
                        for corr in spltcorr:
                            if preflagtargetbeamsmanualflagcorr[int(vis.split('/')[-3])].find(corr) == -1:
                                if preflagtargetbeamsmanualflagcorr[int(vis.split('/')[-3])] == '':
                                    ...
                                    preflagtargetbeamsmanualflagcorr[int(vis.split('/')[-3])] = \
                                    preflagtargetbeamsmanualflagcorr[int(vis.split('/')[-3])] + ',' + corr
            else:
                logger.warning('Target dataset not specified. Specified correlation(s) for target beam dataset(s) will '
                               'not be flagged! #')
    ...
    def manualflag_baseline(self):
        ...
        preflagfluxcalmanualflagbaseline = get_param_def(self, 'preflag_fluxcal_manualflag_baseline',
                                                         np.full((1), '', dtype='U50'))  # Flagged baselines of fluxcal?
        preflagpolcalmanualflagbaseline = get_param_def(self, 'preflag_polcal_manualflag_baseline',
                                                        np.full((1), '', dtype='U50'))  # Flagged baselines of polcal?
        preflagtargetbeamsmanualflagbaseline = get_param_def(self, 'preflag_targetbeams_manualflag_baseline',
                                                             np.full((beams), '',
                                                                     dtype='U50'))  # Flagged baselines of target beams?
        ...
        if self.preflag_manualflag_baseline != '':
            ...
            if preflagfluxcalmanualflagbaseline[
                0] == self.preflag_manualflag_baseline and self.preflag_manualflag_fluxcal == True:
                logger.info(
                    'Baseline(s) ' + self.preflag_manualflag_baseline + ' for flux calibrator were already flagged #')
            else:
                if self.preflag_manualflag_fluxcal and os.path.isdir(
                        self.basedir + '00' + '/' + self.rawsubdir + '/' + self.fluxcal):
                    fc_baseline = 'flagdata(vis="' + self.basedir + '00' + '/' + self.rawsubdir + '/' + self.fluxcal + \
                                  '", antenna="' + self.preflag_manualflag_baseline + '", flagbackup=False)'
                    casacmd = [fc_baseline]
                    casa = drivecasa.Casapy()
                    casa.run_script(casacmd, raise_on_severe=False, timeout=1800)
                    ...
                    for baseline in spltbaseline:
                        if preflagfluxcalmanualflagbaseline[0].find(baseline) == -1:
                            if preflagfluxcalmanualflagbaseline[0] == '':
                                ...
                                preflagfluxcalmanualflagbaseline[0] = preflagfluxcalmanualflagbaseline[
                                                                          0] + ',' + baseline
                else:
                    logger.warning(
                        'No flux calibrator dataset specified. Specified baselines(s) for flux calibrator dataset will not be flagged! #')
            ...
            if preflagpolcalmanualflagbaseline[
                ...
                logger.info(
                    'Baseline(s) ' + self.preflag_manualflag_baseline + ' for polarised calibrator were already flagged #')
            else:
                if self.preflag_manualflag_polcal and os.path.isdir(
                        self.basedir + '00' + '/' + self.rawsubdir + '/' + self.polcal):
                    pc_baseline = 'flagdata(vis="' + self.basedir + '00' + '/' + self.rawsubdir + '/' + self.polcal + \
                                  '", antenna="' + self.preflag_manualflag_baseline + '", flagbackup=False)'
                    casacmd = [pc_baseline]
                    casa = drivecasa.Casapy()
                    casa.run_script(casacmd, raise_on_severe=False, timeout=1800)
                    logger.debug(
                        'Flagged baseline(s) ' + self.preflag_manualflag_baseline + ' for polarised calibrator #')
                    spltbaseline = self.preflag_manualflag_baseline.split(',')
                    for baseline in spltbaseline:
                    ...
                    logger.warning('No polarised calibrator dataset specified. Specified baselines(s) for polarised '
                                   'calibrator dataset will not be flagged! #')
            ...
            if self.preflag_manualflag_target:
                if self.preflag_manualflag_targetbeams == 'all':
                    ...
                    datasets = [self.basedir + str(b).zfill(2) + '/' + self.rawsubdir + '/' + self.target for b in
                                beams]
                    logger.debug(
                        'Flagging baseline(s) ' + self.preflag_manualflag_baseline + ' for selected target beams #')
                for vis in datasets:
                    if preflagtargetbeamsmanualflagbaseline[ int(vis.split('/')[-3])] == self.preflag_manualflag_baseline:
                        logger.info(
                            'Correlation(s) ' + self.preflag_manualflag_baseline + ' for target beam ' + vis.split('/')[
                                -3] + ' were already flagged #')
                    else:
                        logger.debug('Flagging baseline(s) ' + self.preflag_manualflag_baseline + ' for target beam ' +
                                     vis.split('/')[-3] + ' #')
                        tg_baseline = 'flagdata(vis="' + str(
                            vis) + '", antenna="' + self.preflag_manualflag_baseline + '", flagbackup=False)'
                        casacmd = [tg_baseline]
                        casa = drivecasa.Casapy()
                        casa.run_script(casacmd, raise_on_severe=False, timeout=1800)
                        for baseline in spltbaseline:
                            if preflagtargetbeamsmanualflagbaseline[int(vis.split('/')[-3])].find(baseline) == -1:
                                if preflagtargetbeamsmanualflagbaseline[int(vis.split('/')[-3])] == '':
                                    ...
                                    preflagtargetbeamsmanualflagbaseline[int(vis.split('/')[-3])] = \
                                    preflagtargetbeamsmanualflagbaseline[int(vis.split('/')[-3])] + ',' + baseline
            else:
                logger.warning('Target dataset not specified. Specified baseline(s) for target beam dataset(s) will '
                               'not be flagged! #')
    ...
    def manualflag_channel(self):
        ...
        preflagfluxcalmanualflagchannel = get_param_def(self, 'preflag_fluxcal_manualflag_channel',
                                                        np.full((1), '', dtype='U50'))  # Flagged channels of fluxcal?
        preflagpolcalmanualflagchannel = get_param_def(self, 'preflag_polcal_manualflag_channel',
                                                       np.full((1), '', dtype='U50'))  # Flagged channels of polcal?
        preflagtargetbeamsmanualflagchannel = get_param_def(self, 'preflag_targetbeams_manualflag_channel',
                                                            np.full((beams), '',
                                                                    dtype='U50'))  # Flagged channels of target beams?
        ...
        if self.preflag_manualflag_channel != '':
            ...
            if preflagfluxcalmanualflagchannel[
                0] == self.preflag_manualflag_channel and self.preflag_manualflag_fluxcal == True:
                logger.info(
                    'Channel(s) ' + self.preflag_manualflag_channel + ' for flux calibrator were already flagged #')
            else:
                if self.preflag_manualflag_fluxcal and os.path.isdir(
                        self.basedir + '00' + '/' + self.rawsubdir + '/' + self.fluxcal):
                    fc_channel = 'flagdata(vis="' + self.basedir + '00' + '/' + self.rawsubdir + '/' + self.fluxcal + \
                                 '", spw="0:' + self.preflag_manualflag_channel + '", flagbackup=False)'
                    casacmd = [fc_channel]
                    casa = drivecasa.Casapy()
                    casa.run_script(casacmd, raise_on_severe=False, timeout=1800)
                    ...
                    logger.warning('No flux calibrator dataset specified. Specified channel range(s) for flux '
                                   'calibrator dataset will not be flagged! #')
            # Flag channel(s) for the polarised calibrator
            if preflagpolcalmanualflagchannel[
                0] == self.preflag_manualflag_channel and self.preflag_manualflag_polcal == True:
                logger.info(
                    'Channel(s) ' + self.preflag_manualflag_channel + ' for polarised calibrator were already flagged')
            else:
                if self.preflag_manualflag_polcal and os.path.isdir(
                        self.basedir + '00' + '/' + self.rawsubdir + '/' + self.polcal):
                    pc_channel = 'flagdata(vis="' + self.basedir + '00' + '/' + self.rawsubdir + '/' + self.polcal + \
                                 '", spw="0:' + self.preflag_manualflag_channel + '", flagbackup=False)'
                    casacmd = [pc_channel]
                    casa = drivecasa.Casapy()
                    casa.run_script(casacmd, raise_on_severe=False, timeout=1800)
                    logger.debug(
                        'Flagged channel(s) ' + self.preflag_manualflag_channel + ' for polarised calibrator #')
                    spltchannel = self.preflag_manualflag_channel.split(',')
                    for channel in spltchannel:
                    ...
                    logger.warning('No polarised calibrator dataset specified. Specified channel range(s) for '
                                   'polarised calibrator dataset will not be flagged! #')
            ...
            if self.preflag_manualflag_target:
                if self.preflag_manualflag_targetbeams == 'all':
                    ...
                    datasets = [self.basedir + str(b).zfill(2) + '/' + self.rawsubdir + '/' + self.target for b in
                                beams]
                    logger.debug('Flagging channel(s) ' + self.preflag_manualflag_channel +
                                 ' for selected target beams #')
                for vis in datasets:
                    if preflagtargetbeamsmanualflagchannel[int(vis.split('/')[-3])] == self.preflag_manualflag_channel:
                        logger.info(
                            'Correlation(s) ' + self.preflag_manualflag_channel + ' for target beam ' + vis.split('/')[
                                -3] + ' were already flagged #')
                    else:
                        logger.debug('Flagging channel(s) ' + self.preflag_manualflag_channel + ' for target beam ' +
                                     vis.split('/')[-3] + ' #')
                        tg_channel = 'flagdata(vis="' + str(
                            vis) + '", spw="0:' + self.preflag_manualflag_channel + '", flagbackup=False)'
                        casacmd = [tg_channel]
                        casa = drivecasa.Casapy()
                        casa.run_script(casacmd, raise_on_severe=False, timeout=1800)
                        for channel in spltchannel:
                            if preflagtargetbeamsmanualflagchannel[int(vis.split('/')[-3])].find(channel) == -1:
                                if preflagtargetbeamsmanualflagchannel[int(vis.split('/')[-3])] == '':
                                    ...
                                    preflagtargetbeamsmanualflagchannel[int(vis.split('/')[-3])] = \
                                    preflagtargetbeamsmanualflagchannel[int(vis.split('/')[-3])] + ',' + channel
            else:
                logger.warning('Target dataset not specified. Specified channel range(s) for target beam dataset(s)'
                               'will not be flagged! #')
    ...
    def manualflag_time(self):
        ...
        preflagfluxcalmanualflagtime = get_param_def(self, 'preflag_fluxcal_manualflag_time',
                                                     np.full((1), '', dtype='U50'))  # Flagged time range(s) of fluxcal?
        preflagpolcalmanualflagtime = get_param_def(self, 'preflag_polcal_manualflag_time',
                                                    np.full((1), '', dtype='U50'))  # Flagged time range(s) of polcal?
        preflagtargetbeamsmanualflagtime = get_param_def(self, 'preflag_targetbeams_manualflag_time',
                                                         np.full((beams), '',
                                                                 dtype='U50'))  # Flagged time range(s) of target beams?
        ...
        if self.preflag_manualflag_time != '':
            ...
            if preflagfluxcalmanualflagtime[
                0] == self.preflag_manualflag_time and self.preflag_manualflag_fluxcal == True:
                logger.info('Time range ' + self.preflag_manualflag_time + ' for flux calibrator was already flagged #')
            else:
                if self.preflag_manualflag_fluxcal and os.path.isdir(
                        self.basedir + '00' + '/' + self.rawsubdir + '/' + self.fluxcal):
                    fc_time = 'flagdata(vis="' + self.basedir + '00' + '/' + self.rawsubdir + '/' + self.fluxcal + \
                              '", timerange="' + self.preflag_manualflag_time + '", flagbackup=False)'
                    casacmd = [fc_time]
                    casa = drivecasa.Casapy()
                    casa.run_script(casacmd, raise_on_severe=False, timeout=1800)
                    ...
                    logger.warning('No flux calibrator dataset specified. Specified time range(s) for flux '
                                   'calibrator dataset will not be flagged! #')
            # Flag time range for the polarised calibrator
            if preflagpolcalmanualflagtime[
                0] == self.preflag_manualflag_time and self.preflag_manualflag_polcal == True:
                logger.info(
                    'Time range ' + self.preflag_manualflag_time + ' for polarised calibrator was already flagged #')
            else:
                if self.preflag_manualflag_polcal and os.path.isdir(
                        self.basedir + '00' + '/' + self.rawsubdir + '/' + self.polcal):
                    pc_time = 'flagdata(vis="' + self.basedir + '00' + '/' + self.rawsubdir + '/' + self.polcal + \
                              '", timerange="' + self.preflag_manualflag_time + '", flagbackup=False)'
                    casacmd = [pc_time]
                    casa = drivecasa.Casapy()
                    casa.run_script(casacmd, raise_on_severe=False, timeout=1800)
                    ...
                    logger.warning('No polariased calibrator dataset specified. Specified time range(s) for '
                                   'polarised calibrator dataset will not be flagged! #')
            ...
            if self.preflag_manualflag_target:
                ...
                for vis in datasets:
                    if preflagtargetbeamsmanualflagtime[int(vis.split('/')[-3])] == self.preflag_manualflag_time:
                        logger.info('Time range ' + self.preflag_manualflag_time + ' for target beam ' + vis.split('/')[
                            -3] + ' was already flagged #')
                    else:
                        logger.debug('Flagging time range(s) ' + self.preflag_manualflag_time + ' for target beam ' +
                                     vis.split('/')[-3] + ' #')
                        tg_time = 'flagdata(vis="' + str(
                            vis) + '", timerange="' + self.preflag_manualflag_channel + '", flagbackup=False)'
                        casacmd = [tg_time]
                        casa = drivecasa.Casapy()
                        casa.run_script(casacmd, raise_on_severe=False, timeout=1800)
                        for time in splttime:
                            if preflagtargetbeamsmanualflagtime[int(vis.split('/')[-3])].find(time) == -1:
                                if preflagtargetbeamsmanualflagtime[int(vis.split('/')[-3])] == '':
                                    ...
                                    preflagtargetbeamsmanualflagtime[int(vis.split('/')[-3])] = \
                                    preflagtargetbeamsmanualflagtime[int(vis.split('/')[-3])] + ',' + time
            else:
                logger.warning('Target dataset not specified. Specified time range(s) for target beam dataset(s) will '
                               'not be flagged! #')
    ...
    def aoflagger_bandpass(self):
        """
        Creates a preliminary bandpass for flagging from the flux calibrator and applies it to all calibrators and
        target fields
        """
        subs_setinit.setinitdirs(self)
        beams = 37
        ...
        preflagaoflaggerbandpassstatus = get_param_def(self, 'preflag_aoflagger_bandpass_status',
                                                       False)  # Bandpass successfully derived
        preflagaoflaggerfluxcalbandpassapply = get_param_def(self, 'preflag_aoflagger_fluxcal_bandpass_apply',
                                                             False)  # Bandpass applied to flux calibrator?
        preflagaoflaggerpolcalbandpassapply = get_param_def(self, 'preflag_aoflagger_polcal_bandpass_apply',
                                                            False)  # Bandpass applied to polarised calibrator?
        preflagaoflaggertargetbeamsbandpassapply = get_param_def(self, 'preflag_aoflagger_targetbeams_bandpass_apply',
                                                                 np.full((beams),
                                                                         False))  # Bandpass applied to target beams?

        if self.preflag_aoflagger_bandpass:
            # Check if bandpass was already derived and bandpass table is available
            if preflagaoflaggerbandpassstatus and os.path.isdir(
                    self.basedir + '00' + '/' + self.rawsubdir + '/' + self.fluxcal[:-3] + '_Bpass'):
            ...
            # If not, derive the bandpass
                ...
                if self.fluxcal != '' and os.path.isdir(
                        self.basedir + '00' + '/' + self.rawsubdir + '/' + self.fluxcal):
                    fc_bpass = 'bandpass(vis="' + self.basedir + '00' + '/' + self.rawsubdir + '/' + self.fluxcal + \
                               '", caltable="' + self.basedir + '00' + '/' + self.rawsubdir + '/' + \
                               self.fluxcal[:-3] + '_Bpass")'
                    casacmd = [fc_bpass]
                    casa = drivecasa.Casapy()
                    casa.run_script(casacmd, raise_on_severe=False, timeout=1800)
                    if os.path.isdir(self.basedir + '00' + '/' + self.rawsubdir + '/' + self.fluxcal[:-3] + '_Bpass'):
                        ...
                        logger.error('Preliminary bandpass table for flux calibrator was not derived successfully! #')
            else:
                logger.error('Bandpass table not available. Preliminary bandpass cannot be applied #')
            ...
            if preflagaoflaggerbandpassstatus:
                if self.preflag_aoflagger_fluxcal:
                    if preflagaoflaggerfluxcalbandpassapply:
                        logger.info('Preliminary bandpass was already applied to the flux calibrator #')
                    else:
                        # Apply the bandpass to the flux calibrator
                        if self.fluxcal != '' and os.path.isdir(
                                self.basedir + '00' + '/' + self.rawsubdir + '/' + self.fluxcal):
                            logger.info('Applying preliminary bandpass table to flux calibrator #')
                            fc_apply = 'applycal(vis="' + self.basedir + '00' + '/' + self.rawsubdir + '/' + \
                                       self.fluxcal + '", gaintable=["' + self.basedir + '00' + '/' + \
                                       self.rawsubdir + '/' + self.fluxcal[:-3] + '_Bpass"])'
                            casacmd = [fc_apply]
                            casa = drivecasa.Casapy()
                            casa.run_script(casacmd, raise_on_severe=False, timeout=3600)
                            preflagaoflaggerfluxcalbandpassapply = True
                        else:
                            logger.warning('Flux calibrator dataset not specified or not available. #')
                else:
                    logger.info('Not applying preliminary bandpass to flux calibrator! #')
                if self.preflag_aoflagger_polcal:
                    if preflagaoflaggerpolcalbandpassapply:
                        logger.info('Preliminary bandpass was already applied to the polarised calibrator #')
                    else:
                        # Apply the bandpass to the polarised calibrator
                        if self.polcal != '' and os.path.isdir(
                                self.basedir + '00' + '/' + self.rawsubdir + '/' + self.polcal):
                            logger.info('Applying preliminary bandpass table to polarised calibrator #')
                            pc_apply = 'applycal(vis="' + self.basedir + '00' + '/' + self.rawsubdir + '/' + \
                                       self.polcal + '", gaintable=["' + self.basedir + '00' + '/' + \
                                       self.rawsubdir + '/' + self.fluxcal[:-3] + '_Bpass"])'
                            casacmd = [pc_apply]
                            casa = drivecasa.Casapy()
                            casa.run_script(casacmd, raise_on_severe=False, timeout=3600)
                            ...
                            logger.warning('Polarised calibrator dataset not specified or not available. #')
                else:
                    logger.info('Not applying preliminary bandpass to polarised calibrator! #')
                # Apply the bandpass to the target beams
                if self.preflag_aoflagger_target:
                    if self.target != '':
                        if self.preflag_aoflagger_targetbeams == 'all':
                            datasets = glob.glob(self.basedir + '[0-9][0-9]' + '/' + self.rawsubdir + '/' + self.target)
                            logger.info('Applying preliminary bandpass to all target beams #')
                        else:
                            beams = self.preflag_aoflagger_targetbeams.split(",")
                            datasets = [self.basedir + str(b).zfill(2) + '/' + self.rawsubdir + '/' + self.target for b
                                        in beams]
                            logger.info('Applying preliminary bandpass to all selected target beam(s) #')
                        for vis in datasets:
                            if os.path.isdir(vis):
                                if preflagaoflaggertargetbeamsbandpassapply[int(vis.split('/')[-3])] == False:
                                    logger.info('Applying preliminary bandpass table to target beam ' + vis.split('/')[
                                        -3] + ' #')
                                    tg_apply = 'applycal(vis="' + str(
                                        vis) + '", gaintable=["' + self.basedir + '00' + '/' + self.rawsubdir + \
                                               '/' + self.fluxcal[:-3] + '_Bpass"])'
                                    casacmd = [tg_apply]
                                    casa = drivecasa.Casapy()
                                    ...
                                    logger.info('Preliminary bandpass for target beam ' + vis.split('/')[
                                        -3] + ' was already applied. #')
                            else:
                                logger.warning('Target beam dataset ' + vis.split('/')[-3] + ' not available! #')
                    else:
                        logger.warning('Target dataset not specified! #')
                else:
                    logger.warning('Not applying preliminary bandpass to target beams! #')
            else:
                logger.error('Bandpass table could not be applied #')
        ...
        subs_param.add_param(self, 'preflag_aoflagger_targetbeams_bandpass_apply',
                             preflagaoflaggertargetbeamsbandpassapply)
    ...
    def aoflagger_flag(self):
        ...
        preflagaoflaggerfluxcalflag = get_param_def(self, 'preflag_aoflagger_fluxcal_flag_status',
                                                    False)  # AOFlagged the flux calibrator?
        preflagaoflaggerpolcalflag = get_param_def(self, 'preflag_aoflagger_polcal_flag_status',
                                                   False)  # AOFlagged the polarised calibrator?
        preflagaoflaggertargetbeamsflag = get_param_def(self, 'preflag_aoflagger_targetbeams_flag_status',
                                                        np.full((beams), False))  # AOFlagged the target beams?
        ...
        if self.preflag_aoflagger:
            ...
            if self.preflag_aoflagger_fluxcal:
                if not preflagaoflaggerfluxcalflag:
                    if self.fluxcal != '' and os.path.isdir(
                            self.basedir + '00' + '/' + self.rawsubdir + '/' + self.fluxcal) and self.preflag_aoflagger_fluxcalstrat != '':
                        logger.info('Using AOFlagger to flag flux calibrator dataset #')
                        preflagaoflaggerfluxcalbandpassapply = get_param_def(self,
                                                                             'preflag_aoflagger_fluxcal_bandpass_apply',
                                                                             False)  # Check if bandpass was applied successfully
                        if self.aoflagger_bandpass and preflagaoflaggerfluxcalbandpassapply:
                            os.system(
                                'aoflagger -strategy ' + ao_strategies + self.preflag_aoflagger_fluxcalstrat +
                                ' -column CORRECTED_DATA ' + self.basedir + '00' + '/' + self.rawsubdir + '/' +
                                self.fluxcal)
                            ...
                            os.system(
                                ...
                                ' ' + self.basedir + '00' + '/' + self.rawsubdir + '/' + self.fluxcal)
                            logger.warning(
                                'Used AOFlagger to flag flux calibrator without preliminary bandpass applied. Better '
                                'results are usually obtained with a preliminary bandpass applied. #')
                            preflagaoflaggerfluxcalflag = True
                        ...
                        logger.error(
                            'Flux calibrator dataset or strategy not defined properly or dataset not available. Not '
                            'AOFlagging flux calibrator. #')
                else:
            ...
            if self.preflag_aoflagger_polcal:
                if preflagaoflaggerpolcalflag == False:
                    if self.polcal != '' and os.path.isdir(
                            self.basedir + '00' + '/' + self.rawsubdir + '/' + self.polcal) and self.preflag_aoflagger_polcalstrat != '':
                        logger.info('Using AOFlagger to flag polarised calibrator dataset #')
                        # Check if bandpass was applied successfully
                        preflagaoflaggerpolcalbandpassapply = get_param_def(self,
                                                                            'preflag_aoflagger_polcal_bandpass_apply',
                                                                            False)
                        if self.aoflagger_bandpass and preflagaoflaggerpolcalbandpassapply:
                            os.system(
                                'aoflagger -strategy ' + ao_strategies + self.preflag_aoflagger_polcalstrat +
                                ' -column CORRECTED_DATA ' + self.basedir + '00' + '/' + self.rawsubdir +
                                '/' + self.polcal)
                            logger.debug(
                                'Used AOFlagger to flag polarised calibrator with preliminary bandpass applied #')
                            ...
                            os.system(
                                ...
                                ' ' + self.basedir + '00' + '/' + self.rawsubdir + '/' + self.polcal)
                            logger.warning(
                                'Used AOFlagger to flag polarised calibrator without preliminary bandpass applied. '
                                'Better results are usually obtained with a preliminary bandpass applied. #')
                            preflagaoflaggerpolcalflag = True
                        elif self.aoflagger_bandpass == False:
                            os.system(
                                'aoflagger -strategy ' + ao_strategies + self.preflag_aoflagger_polcalstrat +
                        ...
                        logger.error(
                            'Polarised calibrator dataset or strategy not defined properly or dataset not available. '
                            'Not AOFlagging polarised calibrator. #')
                else:
            ...
            if self.preflag_aoflagger_target:
                if self.target != '' and self.preflag_aoflagger_targetstrat != '':
                    ...
                    preflagaoflaggertargetbeamsbandpassapply = get_param_def(self,
                                                                             'preflag_aoflagger_targetbeams_bandpass_apply',
                                                                             np.full((beams),
                                                                                     False))
                    ...
                    for vis in datasets:
                        if preflagaoflaggertargetbeamsflag[int(vis.split('/')[-3])] == False:
                            if self.aoflagger_bandpass and \
                                    preflagaoflaggertargetbeamsbandpassapply[int(vis.split('/')[-3])]:
                                os.system('aoflagger -strategy ' + ao_strategies +
                                          self.preflag_aoflagger_targetstrat + ' -column CORRECTED_DATA ' + vis)
                                logger.debug('Used AOFlagger to flag target beam ' +
                                             vis.split('/')[-3] + ' with preliminary bandpass applied #')
                                preflagaoflaggertargetbeamsflag[int(vis.split('/')[-3])] = True
                            elif self.aoflagger_bandpass == True and \
                                    preflagaoflaggertargetbeamsbandpassapply[int(vis.split('/')[-3])] == False:
                                os.system('aoflagger -strategy ' + ao_strategies +
                                          self.preflag_aoflagger_targetstrat + ' ' + vis)
                                logger.warning('Used AOFlagger to flag target beam ' + vis.split('/')[-3] +
                                               ' without preliminary bandpass applied. Better results are usually '
                                               'obtained with a preliminary bandpass applied. #')
                                preflagaoflaggertargetbeamsflag[int(vis.split('/')[-3])] = True
                            elif self.aoflagger_bandpass == False:
                                os.system('aoflagger -strategy ' + ao_strategies +
                                          self.preflag_aoflagger_targetstrat + ' ' + vis)
                            ...
                            logger.info('Target beam ' + vis.split('/')[-3] + ' was already flagged with AOFlagger! #')
                else:
                    logger.error('Target beam dataset(s) or strategy not defined properly. Not AOFlagging target '
                                 'beam dataset(s). #')
    ...
    def reset(self):
        ...
        """
        subs_setinit.setinitdirs(self)
        logger.warning('Deleting all raw data products and their directories. You will need to start with'
                       ' the PREPARE step again!')

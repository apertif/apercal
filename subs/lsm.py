...
def calc_SI(cat1, cat2, limit):
	...
	try: # Handle the exception if the WENSS query did not give any results.
		coords1 = SkyCoord(ra=cat1.RA, dec=cat1.DEC, unit=(u.deg, u.deg))  # Convert the coordinates of the two source catalogues to the right format
		coords2 = SkyCoord(ra=cat2.RA, dec=cat2.DEC, unit=(u.deg, u.deg))
		idx, d2d, d3d = coords1.match_to_catalog_sky(coords2) # Get the indices of the matches (idx), and their distance on the sky (d2d)
		...
		si[nomatch] = -0.7
		si[match] = src_si
		si[si < -3] = -0.7; si[si > 2] = -0.7  # Change the value to -0.7 in case of high absolute values. Maybe wrong source match or variable source
		cat = mplab.rec_append_fields(cat1,'SI',si,dtypes=float)
	except:
		cat = mplab.rec_append_fields(cat1,'SI',-0.7,dtypes=float) # In case the queried area is not covered by WENSS give all sources a spectral index of -0.7.
	return cat
...
def write_mask(outfile, cat):
	...
	for i, x in enumerate(ra_off):
		...
		msktext2 = msktext2 + '1,' + str(int(ra_off[i])) + ',' + str(int(dec_off[i])) + ',' + str(int(majaxis[i])) + ',' + str(int(minaxis[i])) + ',' + str(int(pa[i])) + ','
	msktext = msktext1[:-1] + '\n' + msktext2[:-1]
